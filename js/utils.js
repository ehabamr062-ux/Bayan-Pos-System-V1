        ...
        function checkUpdates() {
            ...
            setTimeout(() => {
                showCustomAlert({
                    ...
                    msg: 'أنت تستخدم حالياً أحدث إصدار مستقر من بَيَان POS (V 2.5.8 النسخة الذهبية).',
                    type: 'success'
                });
            }, 1500);
        }
        ...
        function updateSettingsWarehouseSelects() {
            ...
            if (!select) return;
            select.innerHTML = warehouses.map(w => `<option value="${w.id}" ${w.id == currentWarehouseId ? 'selected' : ''}>${w.name}</option>`).join('');
        }
    ...
    if (currentPlan === 'باقة نسخة المجانية') {
        ...
        if (typeof transactions !== 'undefined') {
            if (actionType === 'invoice') {
                ...
                count = transactions.filter(t => t.type && t.type.includes('قبض') && t.invoiceId).length;
            } else if (actionType === 'disbursement') {
                count = transactions.filter(t => t.type && t.type.includes('صرف') && t.invoiceId).length;
            }
        }
        
        if (actionType === 'invoice' && count >= 200) return false;
        if (actionType === 'receipt' && count >= 75) return false;
        if (actionType === 'disbursement' && count >= 75) return false;
...
window.enforceSubscriptionCheck = function(actionType = 'invoice') {
    if (!window.isSubscriptionValid(actionType)) {
        ...
        if (currentPlan === 'باقة نسخة المجانية') {
            if (actionType === 'invoice') msg = 'لقد استهلكت كامل رصيدك في النسخة المجانية (200 فاتورة).\n\nيرجى الترقية لإحدى الباقات الكاملة لفتح العدد غير المحدود من الفواتير.';
            else msg = 'لقد استهلكت كامل رصيدك في النسخة المجانية للعمليات الحسابية (75 سند).\n\nيرجى الترقية لإحدى الباقات الكاملة لفتح العدد غير المحدود.';
        } else {
        ...
        if (typeof showCustomAlert === 'function') {
            showCustomAlert({
                type: 'error',
                titleText: '🛑 انتهت صلاحية الاشتراك',
                msg: msg,
                confirmText: 'عرض الباقات 💎',
                onConfirm: () => {
...
window.LicenseService = {
    ...
    saveLicenseLocally: async function(plan, expiry) {
        const machineId = localStorage.getItem('bayan_hwid') || localStorage.getItem('bayan_machine_id') || 'LOCAL_DEVICE';
    ...
    verifyLicense: async function() {
        ...
        const expectedSig = await this.generateLicenseSignature(licenseInfo.plan, licenseInfo.expiry, machineId);
