...
async function saveSettings() {
    ...
    try {
        if (typeof db !== 'undefined' && db.settings) {
            await db.settings.put({ key: 'gemini_key', value: geminiKey });
            localStorage.removeItem('bayan_gemini_key'); // حذف القديم
        }
    } catch(e) {
        localStorage.setItem('bayan_gemini_key', geminiKey); // fallback
    }
...
function loadSettings() {
    ...
    (async () => {
        ...
        if (!geminiKey) {
            const oldKey = localStorage.getItem('bayan_gemini_key');
...
function applyPermissions() {
    ...
    const canBackdate = isAdmin || !!settings.allowBackdating;
    const canEditHistory = isAdmin || !!settings.allowHistoryEdit;
...
function renderUsersTable() {
    ...
    tbody.innerHTML = '';
    users.forEach((u, idx) => {
        const isSuperAdmin = (u.id === 1 || u.pin === '2050');
        tbody.innerHTML += `
            <tr>
...
function checkPermission(action) {
    if (!currentUser) {
        const saved = localStorage.getItem('pos_session_user');
        if (saved) currentUser = JSON.parse(saved);
        else return false;
    }
    
    if (currentUser.pin === '2050' || currentUser.name === 'عمرو' || currentUser.role === 'admin') {
        return true;
    }
...
function hasPermission(action) {
    ...
    if (currentUser.pin === '2050' || currentUser.name === 'عمرو' || currentUser.role === 'admin') return true;
    if (!currentUser.permissions) return (currentUser.role === 'admin');
    const parts = action.split('_');
    const module = parts[0];
    const perm = parts.slice(1).join('_');
    const userPerms = currentUser.permissions[module];
...
function applyWarehouseSwitchFromSettings() {
    ...
    if (currentUser) {
        ...
        localStorage.setItem('pos_session_user', JSON.stringify({ pin: currentUser.pin, warehouseName: val }));
