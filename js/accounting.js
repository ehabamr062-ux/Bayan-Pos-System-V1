...
if (finalData.length === 0) tbody.innerHTML = '<tr><td colspan="13" style="text-align:center; padding:20px;">لا توجد بيانات تطابق البحث</td></tr>';
        ...
        function viewInvoiceItems(invoiceId, type = '') {
            if (!checkPermission('docs_view')) return;
            let invoiceItems = transactions.filter(t => t.invoiceId == invoiceId && (type ? t.type === type : true));
            ...
            tx.notes = invoiceItems.find(t => t.notes)?.notes || tx.notes || ''; // ضمان سحب الملاحظات من أي سطر بالفاتورة
            
            tx.items = invoiceItems.filter(i => !i.isInvoiceHead || invoiceItems.length === 1 || i.product).map(i => ({
                name: i.product || i.productName || '-',
                qty: parseFloat(i.qty || 1),
                unit: i.unit || '-',
                price: parseFloat(i.price || 0),
                total: parseFloat(i.total || (parseFloat(i.qty || 1) * parseFloat(i.price || 0)))
            }));
            
            tx.paid = tx.paidAmount || 0;
            if (typeof window.renderCustomInvoiceModal === 'function') {
                window.renderCustomInvoiceModal(tx);
            } else {
        ...
        window.editTransaction = async function(invId, type) {
            ...
            showCustomAlert({
                ...
                onConfirm: async () => {
                    ...
                    editingOriginalDate = { full: t.date, iso: t.dateISO, time: t.timeISO };
                    ...
                    if (mainType.includes('شراء') && !mainType.includes('مرتجع')) section = 'purchase';

                    else if (mainType.includes('مرتجع بيع')) section = 'sales-return';

                    else if (mainType.includes('مرتجع شراء')) section = 'purchase-return';

                    else if (mainType.includes('قبض')) section = 'receipt';

                    else if (mainType.includes('صرف')) section = 'disbursement';

                    else if (mainType.includes('تسوية')) section = 'adjustment';

                    else if (mainType.includes('تحويل')) section = 'transfer';
                    ...
                    setTimeout(() => {
                        ...
                        if (section === 'sales') {
                            ...
                            setTimeout(() => {
                                ...
                                updateTransferToList(); // لتحديث قائمة الوجهة
            ...
            const content = `
                ...
                <div class="print-container">
                    ...
                    <div class="print-header">
                        ...
                        <div class="print-title">إذن تسوية مخزنية</div>
                    ...
                    <table class="print-table">
                        ...
                        <thead><tr><th>الصنف</th><th>الكمية الفعلية</th><th>السعر</th></tr></thead>
        ...
        function renderAnalysisTable() {
            ...
            if (methodFilter !== 'all') {
                ...
                if (methodFilter === 'نقدية') {
                    ...
                    data = data.filter(t => t.method && (t.method.includes('نقدية') || t.method.includes('كاش') || t.method.includes('فودافون')));
            ...
            if (data.length === 0) {
                ...
                tbody.innerHTML = '<tr><td colspan="10" style="text-align:center; padding:20px;">لا توجد مبيعات في هذه الفترة</td></tr>';
            ...
            if (currentAnalysisMode === 'detailed') {
                ...
                data.forEach((t, idx) => {
                    ...
                    const displayProfit = isReturn ? -profit : profit;

                    const profitPercentTotal = totalPeriodProfit > 0 ? ((displayProfit / totalPeriodProfit) * 100).toFixed(1) : 0;

                    const profitPercentCost = costPrice > 0 ? ((profit / (costPrice * qty)) * 100).toFixed(1) : 0;
                    ...
                    tbody.innerHTML += `
                        ...
                        <tr class="${rowClass}" style="cursor:pointer;" onclick="if(event.target.type !== 'checkbox') { if('${t.invoiceId}' !== '-') viewInvoiceItems('${t.invoiceId}'); }">
                ...
                data.forEach(t => {
                    ...
                    if (!groups[t.product]) {
                        ...
                        groups[t.product] = { name: t.product, qty: 0, total: 0, profit: 0 };
                    ...
                    if (isNaN(profit)) {
                        ...
                        profit = (parseFloat(t.price) - cost) * qty;
                ...
                Object.values(groups).forEach((g, idx) => {
                    ...
                    tbody.innerHTML += `
                        ...
                        <tr class="${rowClass}" style="background: rgba(255,255,255,0.5);">
                            ...
                            <td style="color:#64748b;">مجمع مبيعات</td>

                            <td style="color:#64748b;">-</td>

                            <td style="font-weight:bold; color:var(--main-purple); font-size:1rem;">📦 ${g.name}</td>

                            <td style="font-weight:bold; text-align:center;">${g.qty}</td>

                            <td style="color:#64748b;">-</td>

                            <td style="font-weight:bold;">${avgPrice.toFixed(2)}</td>

                            <td class="profit-col" style="color:${isLoss ? '#ef4444' : '#10b981'}; font-weight:900; font-size:1.1rem;">${g.profit.toFixed(2)}</td>

                            <td class="profit-col" style="font-weight:bold;">${profitPercentTotal}%</td>
        ...
        let analysisColumnVisibility = JSON.parse(localStorage.getItem('pos_an_cols') || '{"0":true,"1":true,"2":true,"3":true,"4":true,"5":true,"6":true,"7":true,"8":true,"9":true,"10":true}');
        ...
        function showAnalysisColumnCustomizer() {
            ...
            cols.forEach(c => {
                ...
                html += `<label style="display:flex; align-items:center; gap:8px; cursor:pointer; padding:5px; background:var(--bg-color); border-radius:5px; border:1px solid var(--border-color);">

                            <input type="checkbox" ${checked} onchange="toggleAnalysisColumn(${c.id}, this.checked)"> ${c.name}
            ...
            modal.innerHTML = `
                ...
                <div class="login-box" style="width: 450px; text-align: right; padding: 25px;">

                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid var(--border-color); padding-bottom:10px;">

                        <h3 style="margin:0;">⚙️ تخصيص أعمدة تحليل المبيعات</h3>

                        <button onclick="this.closest('.modal-overlay').remove()" style="background:none; border:none; font-size:1.5rem; cursor:pointer; color:var(--text-primary);">&times;</button>

                    </div>

                    ${html}

                    <button class="action-btn btn-save" style="width:100%; margin-top:15px;" onclick="this.closest('.modal-overlay').remove()">حفظ وإغلاق</button>
        ...
        window.openTransferModal = function(isEdit = false) {
            ...
            if (!isEdit) {
                ...
                if (document.getElementById('transferDate') && window.editingOriginalDate && window.editingOriginalDate.iso) {
                    document.getElementById('transferDate').value = window.editingOriginalDate.iso;
                }
                if (document.getElementById('transferTime') && window.editingOriginalDate && window.editingOriginalDate.time) {
                    document.getElementById('transferTime').value = window.editingOriginalDate.time;
                }
            }
