...
data.forEach(store => {
        ...
        function switchSection(sectionType, fromTab = false, tabId = null) {
            ...
            sectionsWithDateTime.forEach(config => {
                if (targetTab.type === config.s) {
                    ...
                    if (tEl && !tEl.value) {
                        // For history and analysis, both are dates, not date+time
                        if (config.s === 'history' || config.s === 'analysis') {
                            if (!tEl.value) tEl.value = todayNow;
                        } else {
                            tEl.value = timeNow;
                        }
                    }
        ...
        function getMachineId() {
            let mId = localStorage.getItem('bayan_machine_id');
            if (!mId) {
                // توليد كود عشوائي مكون من 12 رمز (أرقام وحروف)
                const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
                mId = 'BYN-';
                for (let i = 0; i < 8; i++) {
                    mId += chars.charAt(Math.floor(Math.random() * chars.length));
                }
                localStorage.setItem('bayan_machine_id', mId);
            }
            return mId;
        }
        ...
        window.renderCustomInvoiceModal = function(tx) {
            ...
            let paidVal = parseFloat(tx.paid || 0);
            let remainingVal = parseFloat(tx.deferred || tx.remaining || (grandTotal - paidVal));
            ...
            document.getElementById('customPrintBtn').onclick = function() {
                if (isTransfer) {
                    if (window.printTransferInvoice) window.printTransferInvoice(tx);
                } else {
                    if (typeof printInvoice === 'function') {
                        printInvoice({
                            invoiceNumber: tx.invoiceId || tx.invoiceNumber || tx.id,
                            invoiceType: tx.type === 'sale' ? 'بيع' : (tx.type === 'purchase' ? 'شراء' : tx.type),
                            date: (tx.date || tx.timestamp || '').split('T')[0],
                            time: (tx.date || tx.timestamp || '').includes('T') ? (tx.date || tx.timestamp || '').split('T')[1].substring(0,5) : '',
                            ...
                            docType: isReturn ? 'general' : (isPurchase ? 'purchase' : 'sales')
                        });
                    } else {
        ...
        function printSelectedInvoice() {
            const activeIdx = (activeTabId && activeTabId.startsWith('invoices') ? selectedInvoiceIndex : selectedHistoryIndex);
            if (activeIdx === null || activeIdx === undefined) {
                 return showCustomAlert({ type: 'warning', titleText: '⚠️ تنبيه', msg: 'يرجى تحديد عملية للطباعة.' });
            }

            const t = transactions[activeIdx];
            const shopName = document.getElementById('shopName')?.value || 'نظام بَيَان';
                ...
                receiptDiv.innerHTML = `
                    <div class="print-container" style="direction:rtl; font-family:'Arial', sans-serif; padding:10px; color:#000; width:100%; box-sizing:border-box;">
                        <div style="text-align:center; border-bottom:2px solid #000; padding-bottom:10px; margin-bottom:15px;">
                            ...
                            <div style="font-size:18px; font-weight:bold; margin-top:5px;">إشعار تفاصيل حركة</div>
                        </div>

                        <div style="font-size:14px; font-weight:bold; margin-bottom:15px; border: 1px solid #000; padding: 10px; border-radius: 5px;">
                            <div style="display:flex; justify-content:space-between; margin-bottom: 8px;">
                                <span>نوع العملية: <span style="font-size:16px;">${t.type}</span></span>
                                <span>رقم المرجع: ${t.invoiceId || '-'}</span>
                            </div>
                            <div style="display:flex; justify-content:space-between; margin-bottom: 8px;">
                                <span>الطرف: ${t.partner || 'بدون'}</span>
                                <span>التاريخ: ${t.date}</span>
                            </div>
                            <div style="display:flex; justify-content:space-between;">
                                <span>المستخدم: ${t.user || '-'}</span>
                            </div>
                        </div>

                        <table style="width:100%; border-collapse:collapse; font-size:14px; margin-bottom:15px;">
                            <thead>
                                <tr style="border:1px solid #000; background-color:#f0f0f0 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
                                    <th style="text-align:right; padding:8px; border:1px solid #000;">الصنف / البيان</th>
                                    <th style="text-align:center; padding:8px; border:1px solid #000;">الكمية</th>
                                    <th style="text-align:center; padding:8px; border:1px solid #000;">السعر</th>
                                    <th style="text-align:center; padding:8px; border:1px solid #000;">الإجمالي</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td style="text-align:right; padding:8px; border:1px solid #000; font-weight:bold;">${t.product || '-'}</td>
                                    <td style="text-align:center; padding:8px; border:1px solid #000;">${t.qty || '-'}</td>
                                    <td style="text-align:center; padding:8px; border:1px solid #000;">${parseFloat(t.price || 0).toFixed(2)}</td>
                                    <td style="text-align:center; padding:8px; border:1px solid #000; font-weight:bold;">${parseFloat(t.total || t.price || 0).toFixed(2)}</td>
                                </tr>
                            </tbody>
                        </table>

                        <div style="text-align:center; margin-top:20px; font-size:12px; border-top:1px dashed #000; padding-top:10px;">
                            <div style="font-weight:bold;">تمت الطباعة بواسطة نظام بَيَان POS</div>
                        </div>
                    </div>
                `;
                window.print();
        ...
        function attemptLogin() {
            ...
            if (!username) return alert("يرجى اختيار مستخدم أو كتابة الاسم يدوياً");

            // البحث عن المستخدم بالاسم في قاعدة البيانات
            const foundUser = users.find(u => u.name === username);

            if (!foundUser) {
                return alert("❌ اسم المستخدم غير موجود بالنظام!");
            }
